...
def mapreduce(infile,
    ...
    if not isinstance(infile, list):
        infile = [infile]
    map_out = chain.from_iterable(
        chain.from_iterable(mapper(*parser(line)) for line in open(f)) for f in infile)
